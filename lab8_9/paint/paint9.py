...
def calculate_romb(x1, y1, x2, y2):
    ...
    midx = (x1 + x2) / 2
    midy = (y1 + y2) / 2
    # Рассчитываем длину половины диагонали как расстояние между точками
    half_diag = ((x2 - x1)*2 + (y2 - y1)*2)*0.5 / 2
    # Находим оставшиеся вершины, предполагая, что диагонали ромба перпендикулярны
    asty = [midx - half_diag, midy]
    en_asty = [midx + half_diag, midy]
